...
def parse_items(html: str) -> list[tuple[str, str, str | None, str | None, str | None, int, str, str | None, str | None]]:
    """(内部ID, レアリティ, 収録パック表記, SEC判別用の注記, 名前の注釈(「（キラバージョン）」等),
    価格, 商品名のうちID表記より前の部分(キャラ名等), 商品画像URL(あれば),
    商品ページURL(あれば)) のリストを返す。

    パック表記・SEC判別用の注記・名前の注釈は無ければNone。「ID[...]」表記が無い商品
    (オリパ・BOX等)や、「《未開封》」から始まる未開封プロモパックの出品は対象外として除く。
    末尾3つは1枚に特定できなかった場合の管理ページ表示用(unresolved_report参照)で、
    価格照合そのものには使わない。商品ページURLは、同じ型番+レアリティで複数の
    出品がある場合に「どの出品が」どのカードかを個別に区別するための一意なキーになる。
    """
    ...
    for li in soup.select("li.list_item_cell"):
        ...
        results.append((model_number, rarity, pack, variant, annotation, price, product_name, image_url, product_url))
    return results
...
def sync_prices(conn=None, delay: float = REQUEST_DELAY_SEC, progress_callback=None) -> dict:
    ...
    try:
        ...
        while page <= min(last_page, MAX_PAGES):
            ...
            for model_number, rarity, pack, variant, annotation, price, product_name, image_url, product_url in parse_items(html):
